...
- **汉堡菜单**：大小 / 音效 / 音量 / 用量模式 / 峰谷文案 / 气泡开关 / 每轮消耗提示 / 开机自启 + 退出。
...
桌宠菜单里勾选「开机自启」即可实现登录 Windows 后自动启动：

- **勾选**：往注册表 `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`
  写入一条名为 `DSWhale` 的值，内容为 `"<pythonw.exe 路径>" "<项目路径>/main.pyw"`，
  登录时自动无控制台运行（与双击 `main.pyw` 等价）。开关状态持久化在
  `~/.dshw-pet/settings.json` 的 `autostartOn` 字段。
- **取消勾选**：删除该注册表项，不再自启。

手动方式（任选其一，效果等价）：

1. 启动文件夹：`Win+R` 输入 `shell:startup` 回车，把 `main.pyw` 的快捷方式放进去。
2. 注册表：在上述 `Run` 键手动新建字符串值（名 `DSWhale`，数据同勾选时写入的命令）。
